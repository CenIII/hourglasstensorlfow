...
#     return np.abs(np.ndarray.astype(np.round((images/2.0+0.5)*255.0),int))
    ...
    def perform_operation(self, images):
        ...
        for image in images:
            ...
            if(cnt==2):
                ...
                im_arr_xy = np.reshape(im_arr[:,:,[0,1]],[-1,2]).transpose()
                im_arr_xy = im_arr_xy-255.0/2.0 # normalize_d2f(im_arr_xy)
                theta = np.radians(rotation)
                c, s = np.cos(theta), np.sin(theta)
                R = np.array(((c,-s), (s, c)))
                im_arr_xy = R.dot(im_arr_xy)
                im_arr_xy = np.ndarray.astype(np.round(im_arr_xy+255.0/2.0),int).clip(min=0) #de_normalize(im_arr_xy)
                im_arr[:,:,[0,1]] = np.reshape(im_arr_xy.transpose(),[im_arr.shape[0],im_arr.shape[1],2])
...
class Translation(Operation):
    ...
    def __init__(self, probability, max_vert_trans=15, max_hori_trans=15):
        """
        Documentation to appear.
        """
        Operation.__init__(self, probability)
        self.max_vert_trans = abs(max_vert_trans)   # Ensure always negative
        self.max_hori_trans = abs(max_hori_trans)  # Ensure always positive
    ...
    def perform_operation(self, images):
        ...
        random_vert = random.randint(-self.max_vert_trans, self.max_vert_trans)
        random_hori = random.randint(-self.max_hori_trans, self.max_hori_trans)

        def do(image,cnt):
            return self.image_shift(image, random_vert, random_hori, cnt)

        augmented_images = []
        cnt = 0
        for image in images:
            augmented_images.append(do(image,cnt))
            cnt = cnt+1
