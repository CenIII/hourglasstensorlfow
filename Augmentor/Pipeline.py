...
class Pipeline(object):
    ...
    def _execute(self, augmentor_image, count, save_to_disk=True):
        """
        ...
        if augmentor_image.ground_truth is not None:
            if isinstance(augmentor_image.ground_truth, list):
                ...
                for image in augmentor_image.ground_truth2:
                    images.append(Image.open(image))
            else:
                images.append(Image.open(augmentor_image.ground_truth))
                images.append(Image.open(augmentor_image.ground_truth2))
        ...
        if save_to_disk:
            ...
            try:
                ...
                for i in range(len(images)):
                    if i == 0:
                        save_name = str(i)+"/"+str(count-1) + "." + (self.save_format if self.save_format else augmentor_image.file_format)#augmentor_image.class_label + "_original_" + os.path.basename(augmentor_image.image_path) + "_" + file_name \
                        ...
                        save_name = str(i)+"/"+str(count-1) + "." + (self.save_format if self.save_format else augmentor_image.file_format) #"_groundtruth_(" + str(i) + ")_" + augmentor_image.class_label + "_" + os.path.basename(augmentor_image.image_path) + "_" + file_name \
                                    #+ "." + (self.save_format if self.save_format else augmentor_image.file_format)
                        images[i].save(os.path.join(augmentor_image.output_directory, save_name))
            except IOError as e:
    ...
    def sample(self, n):
        ...
        while sample_count <= n:
            for augmentor_image in self.augmentor_images:
                if sample_count <= n:
                    self._execute(augmentor_image, sample_count)
    ...
    def shift_random(self, probability):
        if not 0 < probability <= 1:
            raise ValueError(Pipeline._probability_error_text)
        else:
            self.add_operation(Translation(probability=probability))
    ...
    def ground_truth(self, ground_truth_directory, ground_truth_directory2):
        """
        ...
        for augmentor_image_idx in range(len(self.augmentor_images)):
            ...
            ground_truth_image2 = os.path.join(ground_truth_directory2,
                                              self.augmentor_images[augmentor_image_idx].image_file_name)
            if os.path.isfile(ground_truth_image2):
                self.augmentor_images[augmentor_image_idx].ground_truth2 = ground_truth_image2
                num_of_ground_truth_images_added += 1
